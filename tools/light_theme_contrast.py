...
def render_report(values: dict[str, Color], flutter: Path) -> str:
    ...
    lines = [
        ...
        '## 阶段一结果及保留项', '',
        f'- page/card 的对比度由 {ratio_string(card, old_page)} 变为 {ratio_string(card, new_page)}；cardBorder 对白卡为 {ratio_string(values["LightSurfaces.cardBorder"], card)}，对新页面为 {ratio_string(values["LightSurfaces.cardBorder"], new_page)}。描边取 {values["LightSurfaces.cardBorder"].display()}、0.5–1 逻辑像素（v2：主人实机反馈灰底过深后，page 退回 {values["LightSurfaces.page"].display()} 并以加深描边承担分层），视觉差由 before/after 供 Leader 判断。',
        ...
        '## 阶段二 P0 结果及保留项', '',
        f'- 聊天页局部 page/bar 接 LightSurfaces.page（现 {values["LightSurfaces.page"].display()}）；动作色 #005FB8，不改全局主题。次级文字、占位符、功能灰图标按各自承载面接令牌。',
