...
void main() {
  ...
  group('buildSessionSections 定时融流与过滤逻辑', () {
    ...
    test('showCron = true：cron 会话融流进工作区分区，不再产生「定时」独立分区', () {
      final now = DateTime(2026, 8, 20, 12);
      final cron = _buildSession('cron_1', '定时会话', at: now, workspace: '/ws/ws1');
      final pinned = _buildSession('p1', '置顶会话', pinned: true, at: now, workspace: '/ws/ws2');
      ...
      expect(sections.map((s) => s.title).toList(), ['置顶', 'ws1', 'ws2', 'ws3']);
    ...
    test('非 cron 会话按工作区分组：置顶独立在最上方，无工作区归入「其他」', () {
      ...
      final pinned = _buildSession('p1', '置顶会话', pinned: true, at: now, workspace: '/ws/alpha');
      ...
      expect(sections.map((s) => s.title).toList(), ['置顶', 'alpha', 'beta', '其他']);
      expect(sections[0].sessions.map((s) => s.sessionId).toList(), ['p1']);
  ...
  group('sessionListSectionsProvider 派生状态', () {
    ...
    test('搜索模式派生单一「搜索结果」分区（包含命中结果，不拆分为工作区分区）', () async {
      final cronHit = _buildSession('cron_hit', '定时备份任务', workspace: '/ws/backup');
      final normalHit = _buildSession('s_hit', '普通备份任务', workspace: '/ws/other');
  ...
  group('工作区分组契约专项（#146 · A 案）', () {
    ...
    test('1. 两个工作区的会话分到两组且优先使用 workspaceRoots 中的 name 作为组名', () {
      final s1 = _buildSession('s1', '会话 1', at: now, workspace: '/repo/client');
      final s2 = _buildSession('s2', '会话 2', at: now.subtract(const Duration(minutes: 5)), workspace: '/repo/server');
      ...
      final sections = buildSessionSections([s1, s2], workspaceRoots: roots, now: now);
    ...
    test('2. 组按组内最近活动时间倒序排序', () {
      // wsOld 最近活动 2 小时前
      final old1 = _buildSession('old1', '较早会话', at: now.subtract(const Duration(hours: 2)), workspace: '/ws/old');
      // wsNew 最近活动 10 分钟前，但包含一条 5 小时前的会话
      final new1 = _buildSession('new1', '最新会话', at: now.subtract(const Duration(minutes: 10)), workspace: '/ws/new');
      final new2 = _buildSession('new2', '很早会话', at: now.subtract(const Duration(hours: 5)), workspace: '/ws/new');
    ...
    test('3. workspace 为空的会话进「其他」组，固定排在最后，且标记 isOther', () {
      final sNull = _buildSession('s_null', 'null 工作区', at: now, workspace: null);
      final sEmpty = _buildSession('s_empty', '空串工作区', at: now.subtract(const Duration(minutes: 1)), workspace: '');
      final sSpace = _buildSession('s_space', '空白工作区', at: now.subtract(const Duration(minutes: 2)), workspace: '   ');
      final sWs = _buildSession('s_ws', '有工作区', at: now.subtract(const Duration(hours: 1)), workspace: '/ws/my_project');

      final sections = buildSessionSections([sEmpty, sWs, sNull, sSpace], now: now);
      ...
      expect(otherSection.sessions.map((s) => s.sessionId), ['s_null', 's_empty', 's_space']);
    ...
    test('4. workspace 有值但不在 roots 里 → 按路径最后一段独立成组', () {
      final s1 = _buildSession('s1', '自定工作区 1', at: now, workspace: 'D:/random/custom_repo');
      final s2 = _buildSession('s2', '自定工作区 2', at: now.subtract(const Duration(minutes: 10)), workspace: '/var/lib/my_service/');

      // roots 为空，模拟不在 roots 里的任意目录
      final sections = buildSessionSections([s1, s2], workspaceRoots: const [], now: now);

      expect(sections.map((s) => s.title).toList(), ['custom_repo', 'my_service']);
      expect(sections.any((s) => s.title == '其他'), isFalse);
    });

    test('5. 置顶区跨工作区独立置于最上方，不参与工作区分组', () {
      final p1 = _buildSession('p1', '置顶 1', pinned: true, at: now, workspace: '/ws/alpha');
      final p2 = _buildSession('p2', '置顶 2', pinned: true, at: now.subtract(const Duration(minutes: 1)), workspace: '/ws/beta');
      final p3 = _buildSession('p3', '置顶 3（无工作区）', pinned: true, at: now.subtract(const Duration(minutes: 2)), workspace: null);
      final a1 = _buildSession('a1', 'Alpha 正常', at: now, workspace: '/ws/alpha');
      ...
      // 三个置顶会话聚合在最顶部分区
      expect(sections.first.sessions.map((s) => s.sessionId).toList(), ['p1', 'p2', 'p3']);
      // 其余会话正常进入工作区
      expect(sections.sublist(1).map((s) => s.title).toList(), ['alpha', 'beta']);
    });
    ...
    test('容错比对：反斜杠、正斜杠与首尾空格通过 matchesWorkspace 合流同一组', () {
      final s1 = _buildSession('s1', 'Windows 风格路径', at: now, workspace: r'C:\Projects\HermesApp');
      final s2 = _buildSession('s2', 'Unix 风格路径带空格', at: now.subtract(const Duration(minutes: 5)), workspace: '  C:/Projects/HermesApp  ');

      final sections = buildSessionSections([s1, s2], now: now);

      expect(sections.length, 1);
      expect(sections.single.title, 'HermesApp');
      expect(sections.single.sessions.map((s) => s.sessionId).toList(), ['s1', 's2']);
    });

    test('折叠控制器操作：切换、展开、折叠及状态判断', () {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final notifier = container.read(sessionListCollapsedSectionsProvider.notifier);
